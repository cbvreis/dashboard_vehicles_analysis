...
class Chart:
    def create_line_plot(self, df, x, y, color, title):
        '''
    ...
    def create_matrix(self, values, df, filter, x1, x2, x3):
        ...
        fig = go.Figure(data=[go.Table(header=
                                       ...
                                       cells=dict(values=[
                                           ...
                                           df.groupby(filter).agg({x2: 'median'}).round(2),
                                           df.groupby(filter).agg({x3: 'median'}).round(2),
                                       ],
    ...
    def create_matrix_comparation(self, values, df, filter, x1, x2, x3):
        '''
        Function to create matrix plot
        :param values:
        :param df:
